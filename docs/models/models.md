...
- **Restricted VAR**: Equation-level coefficient constraints via masking matrix.
- **Residual Diagnostics Suite**: Serial correlation (Portmanteau), normality (skew/kurtosis), ARCH-LM, and stability roots tests integrated into a unified `diagnostics` module.
- **SVEC (Structural VECM)**: Combining cointegration terms with short-run and long-run structural restrictions.

### Medium Effort
These models introduce new identification paradigms, custom sampling/rotations, or multi-start optimizers.
- **Sign & Zero Restrictions (`SignZeroSVARModel`)**: Configurable YAML schema for shock responses, QR orthogonal rotations, and accepted-draw inference (non-Bayesian set identification).
- **Narrative Restrictions**: Constraining the sign or magnitude of structural shocks and historical contributions during specific historical events.
- **Data-Driven SVAR (Independence/ICA)**: Identification via non-Gaussianity or distance-covariance metrics, requiring permutation alignment and optimizer diagnostics.
...
- **Threshold VAR (TVAR)**: Two-regime threshold search, regime classification, and regime-specific forecasting.
- **Threshold VECM (TVECM)**: TVAR logic applied to cointegrated systems.
- **Smooth Transition VAR (STVAR)**: Bounded logistic transition function optimization.
- **Generalized IRF (GIRF)**: Simulation-based state-dependent impulse responses for non-linear models.
