...
| `tests/data/` | packaged data and `DataMerger` | loading, joins, and data contracts |
| `tests/feature_engineering/` | `FeatureEngineer` | transformations, resampling, entity-aware behavior |
| `tests/models/test_regression_models.py` | OLS, robust OLS, panel OLS, IV, diagnostics | estimator behavior and summary metrics |
| `tests/models/test_discrete_models.py` | logit | binary-model fitting and metrics |
| `tests/models/test_timeseries_models.py` | VAR, VECM, and core time-series methods | estimator and utility behavior |
| `tests/models/test_unsupervised_models.py` | PCA and K-means | component and cluster outputs |
...
| `tests/visualization/` | charts, EDA, model visualizers, event studies | plot and visualizer construction |
...
| Features and data | `tests/feature_engineering/`, `tests/data/` | transformations, resampling, merging, data loading, and packaged examples |
| Regression and discrete models | `tests/models/test_regression_models.py`, `tests/models/test_discrete_models.py` | OLS variants, panel models, IV, diagnostics, and logit behavior |
| Time series | `tests/models/test_timeseries_models.py`, `tests/test_timeseries_extensions.py` | VAR, VECM, Granger utilities, Blanchard--Quah, proxy SVAR, sign restrictions, LP-IV, and decompositions |
| Visualization and reporting | `tests/visualization/`, `tests/reporting/` | chart construction, visualizer behavior, event studies, and exporters |
| Utilities | `tests/utils/` | configuration and country-name conversion |
