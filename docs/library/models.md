...
### 4.1 Principal Component Analysis (`PCAModel`)
- **Description**: Standardized PCA feature extraction producing factor loadings, scree variance shares, and orthogonal latent components.
- **Specification**: Spectral decomposition of correlation matrix $\Sigma_X = V \Lambda V'$.
- **Module Location**: `src/stats_transformer/models/unsupervised/pca.py`
- **Pipeline Access**: `pca` (direct instantiation: `PCAModel`)
...
### 4.2 K-Means Clustering (`KMeansModel`)
- **Description**: Standardized K-means clustering algorithm partitioning observations into $K$ disjoint clusters by minimizing within-cluster sum of squares.
- **Specification**: $\arg\min_S \sum_{i=1}^K \sum_{x \in S_i} \| x - \mu_i \|^2$
- **Module Location**: `src/stats_transformer/models/unsupervised/kmeans.py`
- **Pipeline Access**: `kmeans` (direct instantiation: `KMeansModel`)
...
## 5. Reduced-Form Time Series Models

### 5.1 Vector Autoregression (`VARModel`)
...
### 5.2 Vector Error Correction Model (`VECMModel`)
...
### 5.3 Restricted VAR (`RestrictedVAR`)
...
### 5.4 Univariate ARIMA (`ARIMAModel`)
...
### 5.5 Analytical Conjugate Bayesian VAR (`BVARModel`)
...
### 5.6 Dynamic Factor Model (`DynamicFactorModel`)
...
### 5.7 Local Projections (`LocalProjectionsModel`)
...
### 5.8 Local Projections IV (`LocalProjectionsIVModel`)
...
## 6. Structural Identification (SVAR & SVEC)

### 6.1 Short-Run SVAR (`SVARModel`)
...
### 6.2 Blanchard-Quah Long-Run SVAR (`BlanchardQuahModel`)
...
### 6.3 Proxy SVAR / SVAR-IV (`ProxySVARModel`)
...
### 6.4 Sign, Zero, & Narrative Restrictions (`SignZeroSVARModel`)
...
### 6.5 Changes in Volatility SVAR (`VolatilitySVARModel`)
...
### 6.6 Distance Covariance & ICA SVAR (`IndependenceSVARModel`)
...
### 6.7 Cramér-von Mises Distance SVAR (`CVMSVARModel`)
...
### 6.8 Non-Gaussian Maximum Likelihood SVAR (`NonGaussianSVARModel`)
...
## 7. Non-Linear & Regime-Switching Models

### 7.1 Threshold Vector Autoregression (`TVARModel`)
...
### 7.3 Smooth Transition VAR (`STVARModel`)
...
### 7.4 Generalized Impulse Response Functions (`GIRFEngine`)
...
## 8. Time-Series Diagnostics & Utilities
