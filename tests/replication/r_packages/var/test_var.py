...
def test_var_models_integration():
    # 1. Run R VAR script
    r_script = Path("tests/replication/r_packages/var/var.R")
    subprocess.run(["Rscript", str(r_script)], check=True)
    
    # 2. Read results
    res_file = Path("tests/replication/r_packages/var/var_results.json")
    assert res_file.exists()
    
    with open(res_file, "r") as f:
        r_res = json.load(f)
        
    df = pd.read_csv("tests/replication/r_packages/var/canada.csv")
